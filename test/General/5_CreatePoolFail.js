...
contract("Create Pool Fails", async accounts => {
    it("Fail Open finshed pool", async () => {
        ...
        await truffleAssert.reverts(instance.CreatePool(Token.address, Math.floor(date.getTime() / 1000) - 2000, rate, rate, amount, false, zero_address,true, { from: accounts[0] }));
      });
      ...
      it("No price", async () => {
        ...
        await truffleAssert.reverts(instance.CreatePool(Token.address, Math.floor(date.getTime() / 1000) + 60, rate, rate, amount, false, zero_address,true, { from: accounts[0] }));
      });
      it("Invalid Poz Rate", async () => {
        ...
        let date = new Date();
        date.setDate(date.getDate() + 1);   // add a day
        await truffleAssert.reverts(instance.CreatePool(Token.address, Math.floor(date.getTime() / 1000) + 60, 2*rate, rate, amount, false, zero_address,true, { from: accounts[0] }));
      });
      it("No Poz Rate", async () => {
        ...
        let date = new Date();
        date.setDate(date.getDate() + 1);   // add a day
        await truffleAssert.reverts(instance.CreatePool(Token.address, Math.floor(date.getTime() / 1000) + 60, 0, 0, amount, false, zero_address,true, { from: accounts[0] }));
      });
      it("Poz Rate = 0", async () => {
        let instance = await ThePoolz.new();
        let Token = await TestToken.deployed()
        ...
        let date = new Date();
        date.setDate(date.getDate() + 1);   // add a day
        await truffleAssert.reverts(instance.CreatePool(Token.address, Math.floor(date.getTime() / 1000) + 60, rate, 0, amount, false, zero_address,true, { from: accounts[0] }));
      });
