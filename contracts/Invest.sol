...
contract Invest is PoolsData {
        ...
        ReceivETH(msg.value, msg.sender)
    {
        ...
        if (pools[_PoolId].Is21DecimalRate) {
            WithDiscount = SafeMath.div(WithDiscount, 10**21);
            TokensAmount = SafeMath.div(TokensAmount, 10**21);
        }
        if (
    ...
    function InvestERC20(uint256 _PoolId, uint256 _Amount) external payable {
        ...
        if (
            ...
            uint256 FeePay = SafeMath.mul(SafeMath.div(_Amount, 10000), PozFee);
            ...
            FeeMap[pools[_PoolId].Maincoin] = SafeMath.add(
                ...
                FeePay
            ); //save the fee amount, in case some one put main coin as pool coin
            TransferToken(
            ...
            uint256 RegularPaymentMinusFee = SafeMath.sub(
                _Amount,
                RegularFeePay
            );
            FeeMap[pools[_PoolId].Maincoin] = SafeMath.add(
